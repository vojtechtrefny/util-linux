/*
 * DASD partition table on-disk structures.
 *
 *
...
/*
 * Volume label — merged CDL/LDL layout.
 *
 * CDL: volkey = key (record key), vollbl = "VOL1" in EBCDIC
 * LDL: volkey = "LNX1" or "CMS1" in EBCDIC (no key field on disk)
 */
struct dasd_volume_label {
	char		volkey[4];	/* volume key (CDL) or label ID (LDL) */
	char		vollbl[4];	/* "VOL1" in EBCDIC (CDL only) */
	...
	char		lvtoc[14];
	char		res3[28];
	char		ldl_version;	  /* version, valid for LDL format */
	uint64_t	formatted_blocks; /* valid when ldl_version >= 0xf2 */
} __attribute__ ((packed));

/*
 * Format 1 / Format 8 DSCB — partition descriptor.
 * F1 and F8 share the same layout.
 */
struct dasd_format1_label {
	...
	uint8_t		DS1SCAL1;
	char		DS1SCAL3[3];
	uint8_t		DS1LSTAR[3];	/* ttr_t */
	uint16_t	DS1TRBAL;
	uint16_t	res1;
	struct dasd_extent DS1EXT1;	/* first extent */
...
struct dasd_format4_label {
	...
	uint8_t		DS4SMSFG;
	uint8_t		DS4DEVAC;
	struct dasd_dev_const DS4DEVCT;	/* device constants (geometry) */
	char		DS4AMTIM[8];
	char		DS4AMCAT[3];
	char		DS4R2TIM[8];
	char		res1[5];
	...
	char		res3;
	uint32_t	DS4DCYL;	/* number of logical cyls (large vol) */
	char		res4[2];
	uint8_t		DS4DEVF2;
	char		res5;
} __attribute__ ((packed));
...
/*
 * Decode CCHH for large volumes.
 *
 * On large volumes (>65534 cylinders) the cylinder and head values
 * are encoded differently: the upper 12 bits of hh hold the upper
 * cylinder bits, and only the lower 4 bits of hh are the head number.
 *
 * Note: the cc and hh fields are big-endian on disk.
 */
static inline uint32_t dasd_cchh_get_cc(const struct dasd_cchh *p)
{
	uint32_t cyl;
...
static inline uint16_t dasd_cchh_get_hh(const struct dasd_cchh *p)
{
 ...
 * From s390-tools libvtoc/vtoc.c (MIT licensed).
 */
