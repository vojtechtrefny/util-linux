...
static void dasd_get_dsnam(const struct dasd_format1_label *f1, char *dsnam)
{
	...
	for (i = 0; i < (int) sizeof(f1->DS1DSNAM); i++)
			     ...
			     unsigned int blocksize)
{
	...
	for (blk = blk_per_trk + 1; blk < blk_per_trk + 20 && partno < DASD_MAX_PARTITIONS; blk++) {
		...
		/* only Format 1 and Format 8 are valid partition descriptors */
		...
		const struct blkid_idmag *mag __attribute__((__unused__)))
{
	...
	bool is_cdl = false;
	bool is_ldl = false;
	const struct dasd_volume_label_cdl *cdl;
	const struct dasd_volume_label_ldl *ldl;
	const char *magic;
	int rc;
